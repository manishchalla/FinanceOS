...
function CSVImport({ accounts, categories, onClose, onImported }: {
  ...
  async function doImport() {
    ...
    const payload = rows.map(r => {
      ...
      // Auto-detect type: negative amount = expense, positive = income
      // But respect the user's manual override if they chose explicitly
      const autoType: "income"|"expense" = rawAmt < 0 ? "expense" : "income";
      const type = defType === "auto" ? autoType : defType;
      let date = mapping.date >= 0 ? r[mapping.date] : format(new Date(),"yyyy-MM-dd");
      try { const d = new Date(date); if (!isNaN(d.getTime())) date = format(d,"yyyy-MM-dd"); } catch {}
